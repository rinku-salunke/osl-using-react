    ...
    toast(
      ({ closeToast }) => (
        <div style={{ padding: "4px" }}>
          ...
          >
            Delete <b>{vehicleToDelete.vehicleType}</b>?
            <br />
            <span
              style={{
                fontSize: "12px",
  ...
  const filteredVehicles = useMemo(() => {
    return vehicles.filter((v) => {
      const matchType = vehicleTypeFilter
        ? v.vehicleType.toLowerCase().includes(vehicleTypeFilter.toLowerCase())
        : true;
      const matchStatus =
        status === ""
          ? true
          ...
          ? v.status === true
          : v.status === false;
      const matchSearch = v.vehicleType
        .toLowerCase()
        .includes(search.toLowerCase());
      return matchType && matchStatus && matchSearch;
    });
  }, [vehicles, vehicleTypeFilter, status, search]);

  // ===== Cards Data =====
  const statsData = useMemo(() => {
    const total = vehicles.length;
    const active = vehicles.filter((v) => v.status).length;
    const inactive = total - active;

    return [
      {
        label: "Total Vehicle Type",
        value: total,
        icon: (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[1000]">
          <div className="bg-white rounded-lg shadow-xl w-[480px] p-8">
            ...
            <p className="text-sm text-gray-600 text-center mb-6">
              Why are you editing this vehicle type?
            </p>
...
const AddVehicleForm = ({ initialData = null, isEdit, onSave, onCancel }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      {/* ⭐ Title with bottom border (same as table header) */}
      {/* <h1 className="text-2xl font-bold text-gray-900 mb-6 pb-3 border-b border-gray-300 -mx-6"> */}
      ...
      <form
        ...
        className="bg-white rounded-lg border border-none p-6"
      >
        {/* ===== General Details ===== */}
...
const VehicleDetailsView = ({ vehicle, onBack }) => {
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <button
        onClick={onBack}
